...
/* ================================================================
   換日線 — main.js  v7
   基底：document 7（你最新的版本）
   修正：
   [B1] _updateNotes：filter 移到 for 迴圈外，不再誤觸結算
   [B2] 音樂/譜面不同步：musicStartAudioTime 改在 music.play() 之後
        立刻取 currentTime，確保時間軸對齊
   [B3] CalibrationScene 黑屏：移除重複 preload，改用純色背景，
        避免 texture 未就緒
   [B4] 結算提早觸發的雙重保護：加 gameEnding flag
   全部其他功能（地標、漂流瓶、吉祥物、留言板、Bug回報）完整保留
================================================================ */
...
const PlayerState = {
    ...
    has()      { return this.x !== null; },
};

/* ================================================================
   地標資料表（完整保留 document 7）
================================================================ */
const LANDMARKS = [
    {
        ...
        isWave: true,
        dialog: { title: '海浪', lines: [' '] },
    },
    {
        key: 'cord', x: 1070, y: 1500, scale: 0.4,
        colW: 30, colH: 35, nearDist: 150, interactDist: 190,
        dialog: {
            title: '沙灘藝術...?',
            lines: ['沒有人跟你說來路不明的 QR Cord 不要亂掃嗎?', '我已經提醒過你了。'],
            action: 'open_link', actionLabel: '🌟打開連結',
        },
    },
    {
        key: 'stage', x: 300, y: 1450, scale: 2.5,
        colW: 150, colH: 110, nearDist: 200, interactDist: 240,
        dialog: {
            title: '5/27-5/29 畢業歌音樂會',
            lines: ['這是主要演出場地。', '進入舞台可遊玩音樂節奏遊戲！'],
            action: 'enter', actionLabel: '▶  進入舞台',
        },
    },
    {
        key: 'cocona', x: 980, y: 2100, scale: 0.8,
        colW: 30, colH: 35, nearDist: 150, interactDist: 190,
        dialog: { title: '椰子樹', lines: ['這是一棵椰子樹'] },
    },
    {
        key: 'mailbox', x: 1350, y: 1670, scale: 0.12,
        colW: 40, colH: 55, nearDist: 160, interactDist: 200,
        dialog: {
            title: '神奇海螺(留言板)',
            lines: ['為什麼不告訴神奇海螺呢?', ' ', '(神奇海螺會把秘密告訴所有人。)'],
            action: 'guestbook', actionLabel: '💬 開啟留言板',
        },
    },
    {
        key: 'tree1', x: 1120, y: 2160, scale: 0.8,
        colW: 30, colH: 35, nearDist: 150, interactDist: 190,
        dialog: { title: '海邊的草叢', lines: ['只是普通的草叢，', '撞一撞有機會穿模卡在裡面。'] },
    },
    {
        key: 'tree2', x: 320, y: 2160, scale: 0.8,
        colW: 30, colH: 50, nearDist: 80, interactDist: 100,
        dialog: { title: '另一邊的草叢', lines: ['好像散發奇怪的氣息...?', '感覺不要靠太近比較好...'] },
    },
    {
        key: 'crab', x: 320, y: 2000, scale: 0.8,
        colW: 30, colH: 50, nearDist: 80, interactDist: 100,
        dialog: { title: '螃蟹', lines: ['放心他不會把你丟出蟹堡王。', ' 還有他真的沒熟。'] },
    },
    {
        key: 'sign', x: 540, y: 2090, scale: 0.6,
        colW: 40, colH: 40, nearDist: 140, interactDist: 180,
        dialog: {
            title: '場地公告牌',
            lines: [
                '高雄女中第78屆畢業典禮-換日線',
    ...
    },
    {
        ...
        colW: 35, colH: 50, nearDist: 160, interactDist: 200,
        dialog: {
            title: '可疑的綠色兔子',
            lines: ['發現 bug 或有建議嗎？', '點下面填寫回報，我會努力修修看:)'],
            action: 'bug_report', actionLabel: '📝 填寫回報',
        },
    },
    {
        key: 'sun', x: 1300, y: 1900, scale: 0.2,
        colW: 50, colH: 60, nearDist: 170, interactDist: 210,
        floatAmplitude: 12, floatSpeed: 1.8, hasShadow: true,
        dialog: {
            title: '✨太陽朋友',
            lines: ['想知道更多相關資訊嗎？', '快去追蹤我們的 IG 吧！'],
            action: 'open_ig', actionLabel: '🌟 前往 IG',
        },
    },
    {
        key: 'buttle', x: 300, y: 300, scale: 0.9,
        colW: 35, colH: 50, nearDist: 160, interactDist: 200,
        floatAmplitude: 8, floatSpeed: 1.9, hasShadow: true,
        dialog: {
            title: '漂流瓶',
            lines: ['瓶子裡好像裝著什麼...', '要打開來看看嗎？'],
            action: 'show_card', actionLabel: '💌 打開漂流瓶',
        },
    },
];
...
================================================================ */
const MUSIC_CFG = {
    LANES: 7, NOTE_SPEED: 400, HIT_RATIO: 0.82, KEY_RATIO: 0.13,
    NOTE_W: 0.80, NOTE_H: 16, WIN_PF: 55, WIN_GD: 110, LOOK_AHEAD: 1600,
...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.landmarks = LANDMARKS.map(def => {
            ...
            // 光暈（沒有 colW 的地標跳過）
            const haloW = (def.colW || 30) * 2.8;
            const halo = this.add.ellipse(def.x, def.y-8, haloW, 28, 0x9988ff, 0).setDepth(4);

            // 漂浮陰影
            let shadow = null;
            if (def.hasShadow) {
                shadow = this.add.ellipse(def.x, def.y-4, (def.colW||40)*2.2, 18, 0x000000, 0.25).setDepth(4);
            }
        ...
        const startX = PlayerState.has() ? savedPos.x : this.worldWidth / 2;
        const startY = PlayerState.has() ? savedPos.y : this.worldHeight - 50;
        ...
        this.input.on('pointerdown', (ptr) => {
            ...
            if (ptr.x < this.scale.width*0.5 && ptr.y > this.scale.height*0.55) {
                this.joyActive = true; this.joyPointerId = ptr.id;
                this.baseX = ptr.x; this.baseY = ptr.y;
        ...
        this.interactBtn = this.add.image(
            this.scale.width - 60, this.scale.height * 0.78, 'btn_idle'
        ).setOrigin(1, 0.5).setScrollFactor(0).setDepth(30).setScale(0.25)
         .setInteractive()
    ...
    _dialogAction() {
        ...
        if (action === 'open_ig')    window.open('https://www.instagram.com/kghs_78grad?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==', '_blank');
        if (action === 'show_card')  this._showCard();
        if (action === 'open_link')  window.open('https://youtu.be/dQw4w9WgXcQ?si=nINUW_aP17-9zywm', '_blank');
    }
    ...
    _showCard() {
        ...
        if (!document.getElementById('card-style')) {
            ...
            style.textContent = `
                ...
                @keyframes bobCard { 0%,100%{transform:translateY(0) rotate(-1deg)} 50%{transform:translateY(-8px) rotate(1deg)} }
            `;
            document.head.appendChild(style);
        }
        const overlay = document.createElement('div');
        overlay.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,0.82);display:flex;align-items:center;justify-content:center;z-index:9999;cursor:pointer;animation:fadeInCard 0.3s ease;`;
        overlay.innerHTML = `
            <div style="position:relative;max-width:min(480px,88vw);text-align:center;">
                <img src="assets/card.png" alt="邀請卡" style="width:100%;border-radius:12px;box-shadow:0 0 60px rgba(90,79,255,0.6),0 0 20px rgba(0,0,0,0.8);animation:bobCard 3s ease-in-out infinite;">
                <div style="margin-top:14px;color:#9080cc;font-size:13px;font-family:'Noto Sans TC',monospace;letter-spacing:2px;">點擊任意處關閉</div>
            </div>`;
        overlay.onclick = () => { overlay.remove(); this._cardEl=null; this.dialogOpen=false; };
        document.body.appendChild(overlay);
        this._cardEl = overlay;
    }

    /* ── Bug 回報 ── */
    _buildBugForm() { this._bugFormEl = null; }
    _openBugForm() {
        if (this._bugFormEl) return;
        const overlay = document.createElement('div');
        overlay.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,0.72);display:flex;align-items:center;justify-content:center;z-index:9999;font-family:'Noto Sans TC',monospace;`;
        overlay.innerHTML = `
            <div style="background:#0c0c22;border:1.5px solid #5a4fff;border-radius:10px;padding:28px 32px;width:min(480px,90vw);box-shadow:0 0 40px #5a4fff44;">
                <div style="color:#d4caff;font-size:18px;font-weight:bold;letter-spacing:3px;margin-bottom:6px;">🐇 Bug 回報 / 建議</div>
                ...
                <input id="bug-name" type="text" maxlength="30" placeholder="匿名" style="width:100%;padding:8px 10px;background:#1a1a33;border:1px solid #3a3460;color:#d4caff;font-size:14px;border-radius:5px;outline:none;margin-bottom:12px;">
                <label style="color:#9080cc;font-size:13px;display:block;margin-bottom:4px;">問題或建議 *</label>
                <textarea id="bug-desc" rows="4" maxlength="500" placeholder="請描述 bug 或你的建議..." style="width:100%;padding:8px 10px;background:#1a1a33;border:1px solid #3a3460;color:#d4caff;font-size:14px;border-radius:5px;outline:none;resize:vertical;margin-bottom:16px;"></textarea>
                <div style="display:flex;gap:12px;justify-content:flex-end;">
                    <button id="bug-cancel" style="background:transparent;border:1px solid #333355;color:#555577;padding:8px 20px;border-radius:5px;cursor:pointer;font-size:13px;">取消</button>
                    <button id="bug-submit" style="background:#1a1233;border:1px solid #5a4fff;color:#b3aaff;padding:8px 24px;border-radius:5px;cursor:pointer;font-size:13px;">📤 送出</button>
                </div>
        ...
        const setS = (msg, col='#5fffb8') => { const el=document.getElementById('bug-status'); if(el){el.textContent=msg;el.style.color=col;} };
        ...
        document.getElementById('bug-submit').onclick = async () => {
            ...
            if (!desc) { setS('請填寫問題描述！','#ff7eb3'); return; }
            document.getElementById('bug-submit').disabled = true;
            setS('送出中…','#ffe066');
            const r = await GSheets.post(API.BUG_URL, { type:'bug', name, desc, time:new Date().toLocaleString('zh-TW'), ua:navigator.userAgent.substring(0,80) });
            if (r.reason==='no_url') { setS('⚠️ API URL 未設定','#ffe066'); setTimeout(()=>this._closeBugForm(),1800); }
            else { setS('✓ 送出成功！感謝回報 🐇'); setTimeout(()=>this._closeBugForm(),1500); }
        };
    }
    _closeBugForm() { if(this._bugFormEl){this._bugFormEl.remove();this._bugFormEl=null;} this.dialogOpen=false; }
    ...
    update(time) {
        ...
        const speed=5, prevX=this.player.x, prevY=this.player.y;
        ...
        // AABB 碰撞
        for (const lm of this.landmarks) {
            if (!lm.colW) continue; // 沒有碰撞框的地標跳過
        ...
        for (const lm of this.landmarks) {
            ...
            // 漂浮動畫
            if (lm.floatAmplitude && lm.sprite?.active) {
                const phase  = t * lm.floatSpeed + lm._floatOffset;
                const floatY = Math.sin(phase) * lm.floatAmplitude;
                lm.sprite.y  = lm.y + floatY;
                if (lm.shadow?.active) {
                    const sc = 1 - Math.abs(floatY) / (lm.floatAmplitude * 3);
                    lm.shadow.setScale(sc, 1).setAlpha(0.25 * sc);
...
class MusicScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.gameEnding  = false; // [B4] 防止雙重觸發結算
    ...
    /* ── [B2 FIX] 開始遊戲：play() 之後立刻取時間，確保同步 ── */
    _startGame() {
        if (!this.noteQueue.length) { console.log('加載中...'); return; }
        if (!this.music) { console.warn('music 未就緒'); return; }
        this.prepareUI.destroy();
        this.input.off('drag');

        this.music.play();
        // ★ 關鍵修正：play() 呼叫完立刻取 currentTime
    ...
    _createPrepareUI() {
        ...
        /* ① 延遲校準格子 */
        const calibX1=W*0.60, calibX2=W*0.935, calibY1=H*0.17, calibY2=H*0.285;
        const calibCX=(calibX1+calibX2)/2, calibCY=(calibY1+calibY2)/2;
        const savedOfs = parseInt(localStorage.getItem('timingOffset'))||0;
        this.calibOfsText = this.add.text(calibX2-40, calibY1+40,
        ...
        /* ② 速度滑桿格子 */
        const speedX1=W*0.575,speedX2=W*0.895,speedY1=H*0.315,speedY2=H*0.510;
        const speedCX=(speedX1+speedX2)/2,speedCY=(speedY1+speedY2)/2;
        ...
        slTrackG.lineStyle(3,0x4a4d78,1); slTrackG.lineBetween(slX0,slY,slX0+slLen,slY);
        const slFillG=this.add.graphics().setDepth(52);
        const _redrawFill=(px)=>{ slFillG.clear();slFillG.fillStyle(0x7c6fff,0.85);slFillG.fillRect(slX0,slY-3,px-slX0,6); };
        ...
        this.input.on('drag',(pointer,obj,dragX)=>{
            ...
            const cx=Phaser.Math.Clamp(dragX,slX0,slX0+slLen); obj.x=cx;
            const t=(cx-slX0)/slLen; this.speedMultiplier=parseFloat((0.5+t*2.5).toFixed(2));
            this.speedValueTxt.setText(this.speedMultiplier.toFixed(2)+'×'); _redrawFill(cx);
        });

        /* ③ Start 格子 */
        const startX1=W*0.685,startX2=W*0.895,startY1=H*0.705,startY2=H*0.865;
        ...
        /* 加入 container */
        this.prepareUI.add(slTrackG); this.prepareUI.add(slFillG);
        this.prepareUI.add(slHandle); this.prepareUI.add(this.speedValueTxt);
        ...
        this.tweens.add({targets:startGlow,strokeAlpha:0.15,duration:900,yoyo:true,repeat:-1,ease:'Sine.easeInOut'});
    }

    update() {
        if (this.isPreparing || !this.gameReady) return;

        // [B2 FIX] elapsed 以秒為單位的 currentTime 計算
        const elapsed = (this.sound.context.currentTime - this.musicStartAudioTime) * 1000 + this.timingOffset;

        this._spawnNotes(elapsed);
        this._updateNotes(elapsed);   // [B1 FIX] filter 已移到迴圈外
        ...
        if (!this.gameEnding) {
            ...
            const allDone   = this.noteQueue.length === 0 && this.activeNotes.length === 0 && holdsDone;
            const musicDone = this.music && !this.music.isPlaying;
            if (allDone || musicDone) {
                this.gameEnding = true;
                this.gameReady  = false;
                // 給最後一個 note 的動畫補 0.5 秒緩衝
    ...
    _updateNotes(elapsed) {
        ...
        // [B1 FIX] ★ filter 在 for 迴圈結束後才執行，不在迴圈內
...
/* ================================================================
   CalibrationScene — [B3 FIX] 移除重複 preload，改純色背景
================================================================ */
class CalibrationScene extends Phaser.Scene {
    constructor() { super('CalibrationScene'); }

    preload() {
        // [B3 FIX] 只載入 tick 音效，不再重複載 ui_bg
        // ui_bg 已在 MusicScene 載入過，Phaser texture cache 會保留
        // 但直接用純色背景更穩定
    ...
    create() {
        ...
        // [B3 FIX] 用純色背景，避免 texture 未就緒造成黑屏
        this.add.rectangle(W/2, H/2, W, H, 0x050518);

        // 嘗試顯示 ui_bg（如果已載入就顯示，否則跳過）
        if (this.textures.exists('ui_bg')) {
            this.add.image(W/2, H/2, 'ui_bg').setDisplaySize(W, H).setAlpha(0.45);
        }
        this.add.rectangle(W/2, H/2, W, H, 0x050510, 0.72);
        ...
        /* ── 節拍圓 ── */
        this.beatCircle = this.add.circle(W/2, H*0.42, 80, 0x1a1255);
        ...
        this.add.text(20, 20, '← 返回', {
            fontSize:'18px', fill:'#a5e8ff', fontFamily:'monospace',
            backgroundColor:'#0d1a2a', padding:{x:10,y:5},
        }).setInteractive()
          .on('pointerover', function(){ this.setStyle({fill:'#fff'}); })
          .on('pointerout',  function(){ this.setStyle({fill:'#a5e8ff'}); })
          .on('pointerdown', () => {
              if (this.offsetSamples && this.offsetSamples.length >= 2) {
    ...
    _startCalibration() {
        ...
        // 8 拍後自動停止錄入，顯示完成按鈕
        this.time.delayedCall(this.intervalMs * 16, () => {
    ...
    _playBeat() {
        ...
        this.beatCount++;
        this.tweens.add({ targets:this.beatCircle, fillColor:0x3a2aaa, duration:60, yoyo:true, ease:'Quad.easeOut' });
        this.tweens.add({
            targets:this.beatIcon, scaleX:1.3, scaleY:1.3, duration:60, yoyo:true, ease:'Quad.easeOut',
            onComplete: () => {
    ...
    _recordTap() {
        ...
        const beatIdx  = Math.round(elapsed / this.intervalMs);
        const expected = beatIdx * this.intervalMs;
